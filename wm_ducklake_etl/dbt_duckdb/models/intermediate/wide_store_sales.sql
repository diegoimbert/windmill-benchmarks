...
LEFT JOIN {{ ref('stg_customer') }} c ON ss.ss_customer_sk = c.c_customer_sk
JOIN {{ ref('stg_item') }} i ON ss.ss_item_sk = i.i_item_sk
LEFT JOIN {{ ref('stg_store') }} s ON ss.ss_store_sk = s.s_store_sk
LEFT JOIN {{ ref('stg_promotion') }} p ON ss.ss_promo_sk = p.p_promo_sk
LEFT JOIN {{ ref('stg_customer_demographics') }} cd ON ss.ss_cdemo_sk = cd.cd_demo_sk
LEFT JOIN {{ ref('stg_household_demographics') }} hd ON ss.ss_hdemo_sk = hd.hd_demo_sk
