...
LEFT JOIN {{ ref('stg_customer') }} c ON ws.ws_bill_customer_sk = c.c_customer_sk
JOIN {{ ref('stg_item') }} i ON ws.ws_item_sk = i.i_item_sk
LEFT JOIN {{ ref('stg_promotion') }} p ON ws.ws_promo_sk = p.p_promo_sk
LEFT JOIN {{ ref('stg_web_page') }} wp ON ws.ws_web_page_sk = wp.wp_web_page_sk
LEFT JOIN {{ ref('stg_web_site') }} ws2 ON ws.ws_web_site_sk = ws2.web_site_sk
LEFT JOIN {{ ref('stg_ship_mode') }} sm ON ws.ws_ship_mode_sk = sm.sm_ship_mode_sk
LEFT JOIN {{ ref('stg_warehouse') }} w ON ws.ws_warehouse_sk = w.w_warehouse_sk
