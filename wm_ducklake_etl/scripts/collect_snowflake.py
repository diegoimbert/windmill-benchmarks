#!/usr/bin/env python3
"""
Collect timing data from the Snowflake competitor's Airflow DAG run via the REST API.
Same approach as collect_airflow.py but with a different default dag_id.

Usage:
  python collect_snowflake.py --run-id <run_id> [--base-url http://localhost:8080]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Collect Snowflake/Airflow benchmark timings")
    parser.add_argument("--dag-id", default="tpcds_etl_snowflake", help="Airflow DAG ID")
